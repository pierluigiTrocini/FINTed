...
public class OfferServiceImpl implements OfferService {
	...
	public OfferDto save(OfferDto offer) throws ElementNotFoundException, UnavailableException, NoFundException {
		Post post = postDao.findById(offer.getPostId())
				.orElseThrow(() -> new ElementNotFoundException("Post not found"));
		User user = userDao.findById(offer.getUserId())
				.orElseThrow(() -> new ElementNotFoundException("User [user] not found"));
	...
	public void delete(Long id, String username) throws ElementNotFoundException {
		Offer offer = offerDao.findById(id).orElseThrow(() -> new ElementNotFoundException("offer not found"));
		if(offer.getUser().getUsername() != username)
	...
	public List<OfferDto> getPostOffers(Long postId) throws ElementNotFoundException, NullFieldException {
		return offerDao
				.findAllByPost(postDao
						.findById(Optional.ofNullable(postId)
								.orElseThrow(() -> new NullFieldException("No id [post-id] as request param")))
						.orElseThrow(() -> new ElementNotFoundException("Post not found")))
	...
	public List<OfferDto> getUserOffers(Long userId) throws ElementNotFoundException, NullFieldException {
		return offerDao
				.findAllByUser(userDao
						.findById(Optional.ofNullable(userId)
								.orElseThrow(() -> new NullFieldException("No id [user-id] as request param")))
	...
	public List<OfferDto> getSellOffers(Long id) {
		return offerDao.findAllByPostSellerIdAndOfferStatus(id, OfferStatus.PENDING).stream()
	...
	public OfferDto acceptOffer(OfferDto offer) throws ElementNotFoundException {
		User seller = userDao.findById(offer.getPostSellerId())
				.orElseThrow(() -> new ElementNotFoundException("User [seller] not found"));
		Post post = postDao.findById(offer.getPostId())
				.orElseThrow(() -> new ElementNotFoundException("Post not found"));
		Offer _offer = offerDao.findById(offer.getId())
				.orElseThrow(() -> new ElementNotFoundException("Offer not found"));
